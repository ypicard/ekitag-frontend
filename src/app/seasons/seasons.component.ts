...
export class SeasonsComponent {;
  ...
  constructor(public route: ActivatedRoute, private tagApiService: TagApiService) {
    ...
    this.route.data.subscribe(val => {
      ...
      this.currentSeason = this.seasons.find(season => { return season.running; });
    });
  }
  ...
  createSeason(form) {
    ...
    if(this.seasons.map(s => { return s.name }).includes(name)) {
      // No 2 season shall have same
      alert('Name already taken');
      return;
    }
    let nbDays = form.value.maxMatches * 24 * 60 * 60; // Needs to be in seconds
    this.tagApiService.createSeason(name, form.value.maxMatches, form.value.maxTime).subscribe(res => {
  ...
  endSeason(season){
    this.tagApiService.endSeason(season.id).subscribe(res => {
      alert('Season ended !');
      this.updateUI();
    }, err => {
      alert(err.error.message);
